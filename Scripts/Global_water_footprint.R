...
crop_wfp = read_csv("/Users/robinlindstrom/Downloads/Global avg wfp.csv") %>% 
  clean_names() %>% 
  select(item_code = product_code_faostat, item_hs = product_description_hs, green_water,
         blue_water, grey_water)
FAO_codes = read_csv("/Users/robinlindstrom/Downloads/FAOSTAT_data_4-4-2020.csv") %>% 
  clean_names() %>% 
  select(item_code, item)

crop_production_data = read_csv(here("Output processed data", "crop_production_data_processed.csv"))

# Get the crop production data in 2018
