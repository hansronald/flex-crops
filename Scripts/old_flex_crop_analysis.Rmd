...
time_series_category_plot(crop_data, measure = used_measure, index_plot = FALSE, scale = "free_y")
...
p1 = stacked_area_plot(crop_data, category = "Flex crop", crop = "Soybeans",
                       measure = used_measure, cut_off = 0.7)
...
```{r paraguay_crop_production, fig.height = 3, fig.width = 9, echo = FALSE}
...
### HH-index global
```{r global_hh_index, fig.height = 3, fig.width = 9, echo = FALSE}
crop_diversity_HH_index(crop_data, spatial_extension = "global")
```

### HH-index regional
```{r regional_hh_index, fig.height = 6, fig.width = 9, echo = FALSE}
#region_filter1 = c("South Asia", "Sub-Saharan Africa", "Latin America & Caribbean", "Middle East & North Africa")
#region_filter2 = c("Europe & Central Asia", "East Asia & Pacific", "North America")

region1 = unique(crop_data$country_group)[str_detect(unique(crop_data$country_group), "Europe")]
region2 = unique(crop_data$country_group)[str_detect(unique(crop_data$country_group), "Africa")]
region3 = unique(crop_data$country_group)[str_detect(unique(crop_data$country_group), "Asia")]
region4 = unique(crop_data$country_group)[str_detect(unique(crop_data$country_group), "America")]

p1 = crop_diversity_HH_index(crop_data %>% 
                          filter(country_group %in% region1), spatial_extension = "regional")
p2 = crop_diversity_HH_index(crop_data %>% 
                          filter(country_group %in% region2), spatial_extension = "regional")
p3 = crop_diversity_HH_index(crop_data %>% 
                          filter(country_group %in% region3), spatial_extension = "regional")
p4 = crop_diversity_HH_index(crop_data %>% 
                          filter(country_group %in% region4), spatial_extension = "regional")
p5 = crop_diversity_HH_index(crop_data, spatial_extension = "global")
...
### HH-index global
```{r crop_diversity_all_countries, fig.height = 6, fig.width = 9, echo = FALSE}
countries = crop_data %>% distinct(country) %>% pull(country)
country_chunks = split(countries, ceiling(seq_along(countries)/16))

for(i in 1:length(country_chunks)){
  plot_this = crop_diversity_HH_index(crop_data, spatial_extension = "country",
                                      country_var = unlist(country_chunks[i]))
  print(plot_this)
}
```
...
n_items = 5
year_var = 1961:2016
measure_var = c("Area harvested")

p1 = plot_country_crop_data(crop_data, country_var = "Malaysia",
                            measure_var = "Area harvested", cut_off = 0.85, year_var, stacked = TRUE)
p2 = crop_diversity_HH_index(crop_data, spatial_extension = "country", country_var = "Malaysia")

p3 = plot_country_crop_data(crop_data, country_var = "Indonesia",
                            measure_var = "Area harvested", cut_off = 0.85, year_var, stacked = TRUE)
p4 = crop_diversity_HH_index(crop_data, spatial_extension = "country", country_var = "Indonesia")
...
```{r uruguay_crop_production, fig.height = 6, fig.width = 9, echo = FALSE}
...
p1 = plot_country_crop_data(crop_data, country_var = "Uruguay",
                            measure_var = "Area harvested", cut_off = 0.85, year_var, stacked = TRUE)
...
p1 = plot_country_crop_data(crop_data, country_var = "Brazil",
                            measure_var = "Area harvested", cut_off = 0.85, year_var, stacked = TRUE)

p2 = land_use_plot(country = country, stacked = FALSE)

p3 = crop_diversity_HH_index(crop_data, spatial_extension = "country", country_var = "Brazil")
...
p1 = plot_country_crop_data(crop_data, country_var = "Argentina",
                            measure_var = "Area harvested", cut_off = 0.85, year_var, stacked = TRUE)

p2 = land_use_plot(country = country, stacked = FALSE)

p3 = crop_diversity_HH_index(crop_data, spatial_extension = "country", country_var = "Argentina")
