...
trade_data_path = "/Users/robinlindstrom/Google Drive/SRC/Thesis/Data/Trade data FAO/Trade_Crops_Livestock_E_All_Data_NOFLAG.csv"
...
```{r, echo = FALSE}
# Which countries has the highest share of agricultural share of total exports in 2017? ----
...
```{r global_crop_export_analysis, fig.height = 2, fig.width = 7, echo = FALSE}
...
flex_crop_export_share = data.frame(year = 1961:2017, n_countries_with_flex_crop_dominating_export = n_countries_with_flex_crop_dominating_export)

```


```{r flex_crop_dominant_export_plot}
...
export_share_2017 = crop_value_2017 %>% 
  top_n(1, commodity_export) %>% 
  ungroup() %>%
  group_by(iso3_code) %>% 
  summarise(export_share_2017 = sum(commodity_export))
...
national_crop_export_proportion = countries_df %>%
  left_join(export_share_2017, by = "iso3_code") %>%
  left_join(export_share_1961, by = "iso3_code") %>% 
  mutate(change = export_share_2017 - export_share_1961) %>%
  left_join(country_metrics, by = "iso3_code") %>% 
...
# How has the national top exported crop share changed from 1961 to 2017?
...
national_crop_export_proportion %>% 
  ...
  mutate(limit = ifelse(export_share_2017 > 0.1, "high", "low")) %>% 
  filter(!is.na(limit)) %>%
  ggplot(aes(x = export_share_1961, y = export_share_2017, label = iso3_code, color = income)) + 
  geom_point() +
  geom_abline() +
  ...
  #                       export_share_1961 > 0.25 | export_share_2017 > 0.25), hjust=0, vjust=0) +
  # facet_wrap(~limit)
```
...
```{r, echo = FALSE, include = FALSE}
flex_crop_export_share_2017 = export_value %>% 
  filter(year == 2017) %>%
  filter(crop_category == "Flex crops") %>% 
  mutate(individual_crop_proportion_exports = export_value / yearly_national_export) %>% 
  group_by(iso3_code) %>% 
  summarise(flex_crop_export_share_2017 = sum(export_value / yearly_national_export))
...
national_crop_export_proportion = countries_df %>%
  left_join(flex_crop_export_share_2017, by = "iso3_code") %>%
  left_join(flex_crop_export_share_1961, by = "iso3_code") %>% 
  mutate(change = flex_crop_export_share_2017 - flex_crop_export_share_1961) %>%
  left_join(country_metrics, by = "iso3_code") %>% 
  left_join(FAO_codes, by = "iso3_code")
  #gather("type", "value", -c(country, iso3_code))
```
...
national_crop_export_proportion %>% 
  ...
  mutate(limit = ifelse(flex_crop_export_share_2017 < 0.1 & flex_crop_export_share_1961 < 0.1, "low", "high")) %>% 
  filter(!is.na(limit),
         limit == "high") %>%
  ggplot(aes(x = flex_crop_export_share_1961, y = flex_crop_export_share_2017, label = iso3_code, color = country_group)) + 
  geom_point() +
  geom_abline() +
...
# df_export_map_data %>%
#   ggplot(aes(fill = flex_crop_export_share_2017)) +
#   geom_sf() +
#   scale_fill_gradient2() +
