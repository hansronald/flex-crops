...
```{r}
crop_group_regional_harvest = crop_data %>% 
  filter(measures == "Area harvested") %>% 
  group_by(year, crop_category) %>% 
  mutate(total_crop_category_harvest = sum(value)) %>% 
  ungroup() %>% 
  group_by(year, crop_category, country_group) %>% 
  mutate(regional_crop_category_harvest = sum(value)) %>%
  distinct(year, country_group, crop_category, .keep_all = TRUE) %>%
  ungroup() %>% 
  mutate(crop_category_share =  regional_crop_category_harvest / total_crop_category_harvest) %>%
  select(country, iso3_code, crop_category, country_group, year, total_crop_category_harvest, regional_crop_category_harvest, crop_category_share)

# Plot the relative and absolute temporal change in export value of the different crop groups ---
p1 = crop_group_regional_harvest %>% 
...
p2 = crop_group_regional_harvest %>% 
  ggplot(aes(x = year, y = regional_crop_harvest, color = crop_category)) +
  geom_line() +
...
df = crop_data %>% 
  filter(measures == "Area harvested") %>% 
  select(country, iso3_code, year, crop_category, value, item, country_group) %>%
...
```{r flex_crop_total_production2, fig.height = 4, fig.width = 8, echo = FALSE}
df_change = df %>%
  ungroup() %>% 
  ...
  mutate(change_abs = `2016` - `1993`) %>% 
  mutate(change_perc = `2016` / `1993`)
...
crop_proportion_plot_per_country(crop_data, category = "Potential flex crops", min_land_area = 500000, min_proportion = 0.1, max_proportion = 1, stacked = TRUE)
...
crop_proportion_plot_per_country(crop_production_data, category = "Flex crop", min_land_area = 500000, min_proportion = 0.4, max_proportion = 1, stacked = TRUE)
